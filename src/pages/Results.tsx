  ...
  const repaymentStats = [
    {
      ...
      value: `$${mockRepaymentBreakdown.monthlyPayment.toLocaleString()}`,
    },
    {
      icon: Calendar,
      label: "Total Installments",
      value: mockRepaymentBreakdown.numberOfInstallments,
    },
    {
      icon: TrendingUp,
      label: "Total Repayment",
      value: `$${mockRepaymentBreakdown.totalRepayment.toLocaleString()}`,
    },
    {
      icon: Percent,
      label: "Total Interest",
      value: `$${mockRepaymentBreakdown.totalInterest.toLocaleString()}`,
    },
  ];
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Analysis of your loan document from {mockLoanSummary.lender}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                <div className="bg-muted/50 rounded-xl p-3">
                  ...
                  <p className="font-semibold text-foreground">
                    ${mockLoanSummary.loanAmount.toLocaleString()}
                  </p>
                </div>
                <div className="bg-muted/50 rounded-xl p-3">
                  <p className="text-xs text-muted-foreground mb-1">Interest Rate</p>
                  <p className="font-semibold text-foreground">{mockLoanSummary.interestRate}%</p>
                </div>
                <div className="bg-muted/50 rounded-xl p-3">
                  <p className="text-xs text-muted-foreground mb-1">Term Length</p>
                  <p className="font-semibold text-foreground">{mockLoanSummary.termMonths} months</p>
                </div>
                <div className="bg-muted/50 rounded-xl p-3">
                  <p className="text-xs text-muted-foreground mb-1">Loan Type</p>
                  <p className="font-semibold text-foreground">{mockLoanSummary.loanType}</p>
                </div>
              </div>
              <p className="text-muted-foreground leading-relaxed">{mockLoanSummary.summary}</p>
            </ResultCard>
            ...
            >
              ...
              <div className="mt-4 p-4 bg-primary/5 rounded-xl border border-primary/10">
                <p className="text-sm text-muted-foreground">
                  ...
                  {mockRepaymentBreakdown.effectiveAPR}% — This accounts for compound interest and fees.
                </p>
              </div>
            </ResultCard>
                    ...
                    <div className="flex items-start gap-3">
                      <AlertTriangle
                        className={cn(
                          ...
                          term.severity === "high" ? "text-destructive" : "text-warning"
                        )}
                      />
                      <div>
          ...
          <div className="space-y-6">
            <ResultCard
              ...
              icon={Shield}
              variant={getFairnessVariant(mockFairnessScore.score)}
              className={cn(
              ...
              <div className="flex flex-col items-center py-4">
                ...
                <div
                  className={cn(
                    ...
                    getFairnessVariant(mockFairnessScore.score) === "success" &&
                      "bg-success/10 text-success",
                    getFairnessVariant(mockFairnessScore.score) === "warning" &&
                      "bg-warning/10 text-warning",
                    getFairnessVariant(mockFairnessScore.score) === "destructive" &&
                      "bg-destructive/10 text-destructive"
                  )}
                >
                  {getFairnessLabel(mockFairnessScore.score)}
                </div>
              </div>

              {/* Breakdown */}
              <div className="mt-6 space-y-3">
                {mockFairnessScore.breakdown.map((item) => (
                  <div key={item.label}>
